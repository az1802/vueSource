...
const KeepAliveImpl = {
    ...
    setup(props: KeepAliveProps, { slots }: SetupContext) {
        const cache: Cache = new Map() //缓存组件
        ...
        watch( //include,exclude发生变化时对缓存的vnode节点进行处理
        ...
        onBeforeUnmount(() => { //对每一个缓存的vnode节点进行解绑
        ...
        return () => {
            ...
            if (cachedVNode) {
                ...
                // make this key the freshest  根据最新使用的vnode节点保持其排在最后,后续超出缓存移除vnode节点会移除第一个
                keys.delete(key)
                keys.add(key)
            } else {
                keys.add(key)
                // prune oldest entry 超出缓存的最大数量移除第一个位置的vnode
